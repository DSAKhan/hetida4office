...
#define AIN_VCAP_HYST NRF_LPCOMP_HYST_ENABLED
...
FG_ACTOR_SLOT(fg_lpcomp_start)
{
    ...
    nrf_lpcomp_shorts_enable(NRF_LPCOMP, NRF_LPCOMP_SHORT_UP_STOP_MASK);
    nrf_lpcomp_int_enable(NRF_LPCOMP, LPCOMP_INTENSET_READY_Msk);
    nrfx_lpcomp_enable();
}
...
static void lpcomp_irq_handler(nrf_lpcomp_event_t event)
{
    ...
    switch (event)
    {
        case NRF_LPCOMP_EVENT_READY:
            result = nrf_lpcomp_result_get(NRF_LPCOMP);
